...
describe("GET /api/mobile/v1/agents/:id/events", () => {
    test("filters service events and returns mobile transcript fields", async () => {
        const ctx: any = { fns: {
            procs: { db: { select: async ({ sql }: any) => sql.includes("SELECT 1") ? [{ exists: 1 }] : [{ run_state: "idle", next_run_at: null, last_error: null }] } },
            session: {
                getEvents: async () => [{ idx: 1, ts: 10, type: "wake_up", reason: "x" }, { idx: 2, ts: 11, type: "assistant", text: "hello", usage: { tokens: 1 } }],
