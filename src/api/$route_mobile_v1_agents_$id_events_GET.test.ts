...
describe("GET /api/mobile/v1/agents/:id/events", () => {
    test("filters service events, returns mobile fields, and initially requests the newest page", async () => {
        ...
        expect(await response.json()).toMatchObject({ version: 1, agentId: "ab", nextAfter: 3, isRunning: true, partial: { text: "live", revision: 3, startedAt: 9 }, events: [{ idx: 2, type: "assistant", text: "hello" }] });
    });
});
