...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    const icon = status === 'done' ? 'ph-check-circle text-success' : active ? 'ph-circle-notch text-primary' : 'ph-circle text-base-content/25';
    ...
    return `<div data-plan-task data-task-id="${esc(task.id)}" data-task-status="${esc(status)}" class="rounded-lg ${active ? 'border border-ui-border bg-base-100/35 p-2' : 'px-2 py-1'}">
      ...
      <div class="flex items-start gap-2 text-xs"><i class="ph ${icon} mt-2"></i><div class="min-w-0 flex-1">
        <input name="task_title" maxlength="300" value="${esc(task.title ?? '')}" required ${opts.autofocus ? 'autofocus' : ''} placeholder="Task title" aria-label="Task title" class="w-full border-0 bg-transparent p-0 ${status === 'done' ? 'text-base-content/35 line-through' : active ? 'font-medium text-base-content' : 'text-base-content/65'} outline-none focus:ring-0">
        <textarea name="task_instructions" maxlength="12000" rows="${active ? 3 : opts.autofocus ? 2 : 1}" placeholder="Detailed instructions" aria-label="Task instructions" class="mt-1 w-full resize-y border-0 bg-transparent p-0 text-[11px] leading-4 ${active ? 'text-base-content/70' : 'text-base-content/45'} outline-none focus:ring-0">${esc(task.instructions ?? '')}</textarea>
        <code class="block truncate text-[9px] text-base-content/25">${esc(task.id)}</code>
