...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    const teamHtml = allTeam.length ? inspectorSection({
      ...
      <div class="space-y-2">${team.map((member: any) => {
        ...
        return `<details class="group overflow-hidden rounded-lg border border-ui-border bg-base-100" ${member.status === 'working' ? 'open' : ''}>
    ...
    return ctx.fns.ui.live({
      ...
      every: 0,
      tag: 'aside',
      attrs: 'class="flex h-full w-80 shrink-0 flex-col border-l border-ui-border bg-base-100 text-base-content"',
      html: `
      <header class="flex h-8 shrink-0 items-center gap-2 border-b border-ui-border bg-base-200/70 px-3">
