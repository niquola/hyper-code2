...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    const teamHtml = allTeam.length ? inspectorSection({
      ...
      ${archivedTeam.length ? `<label class="mt-2 flex items-center gap-1.5 text-[10px] text-gray-500"><input type="checkbox" onchange="this.closest('details').querySelector('[data-team-archive-list]').classList.toggle('hidden', !this.checked)"> Show archived (${archivedTeam.length})</label>` : ''}
      <div class="space-y-2">${team.map((member: any) => {
        ...
        return `<details class="group overflow-hidden rounded-lg border border-base-300 bg-base-100" ${member.status === 'working' ? 'open' : ''}>
          <summary class="flex min-h-10 cursor-pointer list-none items-center gap-2 px-2.5 py-2 hover:bg-base-200/60"><i class="ph ph-robot text-primary"></i><span class="min-w-0 flex-1 truncate text-xs font-medium text-base-content">${esc(member.title || `Agent ${member.id}`)}</span>${memberBadge}<span class="font-mono text-[10px] tabular-nums text-base-content/45">${memberDone}/${memberTasks.length}</span><i class="ph ph-caret-down text-[10px] text-base-content/35 transition-transform group-open:rotate-180"></i></summary>
          <div class="space-y-2 border-t border-base-300 px-2.5 py-2">
    ...
    </form>${last ? `<div class="mt-3 border-t border-base-300 pt-3 text-xs"><div class="font-medium text-base-content/70">Last check: ${esc(last.status)}</div><div class="mt-1 leading-5 text-base-content/55">${esc(last.reason)}</div>${last.nextStep ? `<div class="mt-1 text-base-content/40">Next: ${esc(last.nextStep)}</div>` : ''}</div>` : ''}<p class="mt-3 text-[10px] leading-4 text-base-content/40">The goal is checked whenever the agent tries to finish. Only “continue” wakes it again; blocked or needs-user stops the run.</p>`;
    ...
    return ctx.fns.ui.live({
      ...
      every: 0,
      tag: 'aside',
      attrs: 'class="flex h-full w-80 shrink-0 flex-col border-l border-base-300 bg-base-100 text-base-content"',
      html: `
      <header class="flex h-8 shrink-0 items-center gap-2 border-b border-base-300 bg-base-200/70 px-3">
