...
export default function (ctx: Context, _session: Session | null, opts: {
    ...
    const teamHtml = allTeam.length ? inspectorSection({
      ...
      <div class="space-y-2">${team.map((member: any) => {
        ...
        return `<details class="group overflow-hidden rounded-lg border border-ui-border-strong bg-base-100" ${member.status === 'working' ? 'open' : ''}>
          ...
          <div class="space-y-2 border-t border-ui-border-strong px-2.5 py-2">
    ...
    </form>${last ? `<div class="mt-3 border-t border-ui-border-strong pt-3 text-xs"><div class="font-medium text-base-content/70">Last check: ${esc(last.status)}</div><div class="mt-1 leading-5 text-base-content/55">${esc(last.reason)}</div>${last.nextStep ? `<div class="mt-1 text-base-content/40">Next: ${esc(last.nextStep)}</div>` : ''}</div>` : ''}<p class="mt-3 text-[10px] leading-4 text-base-content/40">The goal is checked whenever the agent tries to finish. Only “continue” wakes it again; blocked or needs-user stops the run.</p>`;
    ...
    return ctx.fns.ui.live({
      ...
      every: 0,
      tag: 'aside',
      attrs: 'class="flex h-full w-80 shrink-0 flex-col border-l border-ui-border-strong bg-base-100 text-base-content"',
      html: `
      <header class="flex h-8 shrink-0 items-center gap-2 border-b border-ui-border-strong bg-base-200/70 px-3">
