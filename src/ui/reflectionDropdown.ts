...
export default async function (ctx: Context, _session: Session | null, opts: {
    ...
    const contentHtml = `<div class="mb-2 text-[10px] font-semibold uppercase tracking-wide text-base-content/40">What we are doing</div>
        ...
        ${reflection.reflectionNudge?.text ? `<div class="mt-3 rounded-md border border-ui-border bg-base-200 px-3 py-2.5"><div class="flex items-start gap-2"><div class="min-w-0 flex-1"><div class="text-[10px] font-semibold uppercase tracking-wide text-base-content/55">Reflection nudge · ${Number(reflection.reflectionNudge.expiresAfterTurns ?? 3)} turns</div><div class="mt-1 text-base-content/80">${esc(reflection.reflectionNudge.text)}</div>${reflection.reflectionNudge.reason ? `<div class="mt-1 text-base-content/55">${esc(reflection.reflectionNudge.reason)}</div>` : ''}</div>${ctx.fns.procs.ui.button({ action: 'dismiss-reflection-nudge', html: '<i class="ph ph-x"></i>', appearance: 'plain', post: `/agent/${encodeURIComponent(agent.id)}/reflection-nudge/delete`, target: 'closest div.mt-3', swap: 'outerHTML', title: 'Dismiss reflection nudge', ariaLabel: 'Dismiss reflection nudge', class: '-mr-1 -mt-1 inline-flex size-6 shrink-0 items-center justify-center rounded text-violet-400 hover:bg-base-300 hover:text-base-content' })}</div></div>` : ''}
