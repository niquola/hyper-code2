export default function (ctx: Context, _session: Session | null, opts: { agent: types.agent.Agent }): string {
    ...
    const poll = `${idAttr} hx-get="/agent/${encodeURIComponent(agent.id)}/reflection" hx-trigger="hyper-tick from:body, every 60s" hx-swap="outerHTML"`;
    ...
    if (active) return `<span ${poll} class="${buttonClass} border-violet-200 bg-violet-50 text-violet-700" title="Reflection is running" aria-label="Reflection is running"><i class="ph ph-brain animate-spin" aria-hidden="true"></i></span>`;
    if (!reflection) return `<span ${poll} class="${buttonClass} border-gray-200 bg-white text-gray-400" title="Reflection appears after 3 user messages" aria-label="Reflection pending"><i class="ph ph-brain" aria-hidden="true"></i></span>`;
