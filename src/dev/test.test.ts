...
// A small, fast, self-contained test file to point the runner at.
const TARGET = './src/git/status.test.ts';

describe("dev test helpers", () => {
    test("returns compact summary for passing test file", async () => {
        const agent = { scratchpad: {} };
        const res = await devTest({} as Context, null, { files: [TARGET], agent });
    ...
    test("readLog reads last run tail", async () => {
        ...
        const run = await devTest({} as Context, null, { files: [TARGET], agent });
        const log = await readLog({} as Context, null, { run: 'last', tail: 5, agent });
    ...
    test("testSummary returns concise parsed result", async () => {
        const agent = { scratchpad: {} as any };
        await devTest({} as Context, null, { files: [TARGET], agent });
        const summary = await testSummary({} as Context, null, { run: 'last', agent });
    ...
    test("verbose mode returns tails, not full logs", async () => {
        const res = await devTest({} as Context, null, { files: [TARGET], verbose: true });
        expect(res.ok).toBe(true);
