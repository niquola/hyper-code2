...
describe('session append helpers', () => {
  test('append role-specific messages/events', async () => {
    ...
    await ctx.fns.session.appendAssistantMessage({ id: 'a1', msg: { content: 'a' } });
    ...
    expect((await ctx.fns.session.getMessages({ id: 'a1' })).map((m: any) => m.role)).toEqual(['user', 'assistant']);
    expect((await ctx.fns.session.getEvents({ id: 'a1' })).map((e: any) => e.type)).toEqual(['user', 'thinking', 'tool_call', 'assistant', 'error']);
  });
});
