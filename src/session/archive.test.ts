...
describe('session.archive', () => {
    test('archived session disappears from list and cannot be loaded', async () => {
        ...
        const ctx: any = { state: {}, env: {}, fns: { db: { connect, migrate, select: (await import('../db/select')).default, exec: (await import('../db/exec')).default }, session: { save, list, archive, load, appendUserMessage, appendMessage, appendEvent }, agent: { start, renderEventHtml: async () => '' }, events: { emitAgentsChanged: () => {} } } };
