...
async function seed() {
    ...
    const agent = ctx.fns.agent.start({ model: "mock:x" });
    const id = agent.id;
    ctx.fns.session.appendMessage({ id, message: { role: "user", content: "hello" } });               // 0
    ctx.fns.session.appendMessage({ id, message: { role: "assistant", content: "hi prose" } });        // 1
    ctx.fns.session.appendMessage({ id, message: { role: "assistant", content: "§eval\nx=1" } });      // 2
    ctx.fns.session.appendMessage({ id, message: { role: "user", content: "§result:eval\nout", excluded_from_cursor: true } }); // 3
    ctx.fns.session.appendMessage({ id, message: { role: "assistant", content: "done" } });            // 4

    ctx.fns.session.appendEvent({ id, event: { type: "user", text: "hello", messageIdx: 0 } });        // ev0
    ctx.fns.session.appendEvent({ id, event: { type: "assistant", text: "hi prose", messageIdx: 1 } });// ev1
    ctx.fns.session.appendEvent({ id, event: { type: "tool_call", name: "eval", messageIdx: 2 } });    // ev2
    ctx.fns.session.appendEvent({ id, event: { type: "assistant", text: "done", messageIdx: 4 } });    // ev3
    return { ctx, id };
}
...
describe("session.truncateMessagesFrom", () => {
    test("deletes message at idx + its event, keeps earlier rows AND their flags", async () => {
        ...
        const res = ctx.fns.session.truncateMessagesFrom({ id, from: 4 });
    ...
    test("walks back a marker pair: truncating from §result drops the §eval too", async () => {
        const { ctx, id } = await seed();
        const res = ctx.fns.session.truncateMessagesFrom({ id, from: 3 }); // §result → walk back to §eval (2)
    ...
    test("does NOT drop excluded_from_llm rows that survive the cut", async () => {
        ...
        const agent = ctx.fns.agent.start({ model: "mock:x" });
        const id = agent.id;
        ctx.fns.session.appendMessage({ id, message: { role: "user", content: "keep me", excluded_from_llm: true } }); // 0
        ctx.fns.session.appendMessage({ id, message: { role: "user", content: "cut me" } });                            // 1
        ctx.fns.session.truncateMessagesFrom({ id, from: 1 });
        const msgs = rawMsgs(ctx, id);
    ...
    test("invalid idx (beyond end) is rejected", async () => {
        const { ctx, id } = await seed();
        expect(ctx.fns.session.truncateMessagesFrom({ id, from: 99 })).toEqual({ ok: false, reason: "invalid idx" });
    });
});
