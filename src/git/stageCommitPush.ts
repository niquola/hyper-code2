export default async function (
    ...
    const staged = await ctx.fns.git.stage(ctx, opts.paths, { dir: opts.dir });
    const committed = await ctx.fns.git.commit(ctx, opts.message, { dir: opts.dir, allowEmpty: opts.allowEmpty });
