export default async function (ctx: Context) {
    ...
    const server = Bun.serve({
        ...
        async fetch(req) {
            ...
            if (!m) {
                log(req.method, url.pathname + url.search, 404, performance.now() - t0);
                return new Response("Not Found", { status: 404 });
            }
            (req as any).params = m.params;
            try {
                const res = await m.handler(ctx, null, req);
                log(req.method, url.pathname + url.search, res.status, performance.now() - t0);
                return res;
            } catch (e: any) {
                log(req.method, url.pathname + url.search, 500, performance.now() - t0, e?.message);
                throw e;
            }
        },
    });
...
function log(method: string, path: string, status: number, ms: number, err?: string) {
