...
describe('settings declared registry + resolution', () => {
    ...
    test('source = default when nothing in DB or env', async () => {
        const ctx = await mkTestCtx();
        const item = ctx.fns.settings.declared({}).find((i: any) => i.key === 'defaultModel');
    ...
    test('source = env when env var set, no DB row', async () => {
        ...
        const item = ctx.fns.settings.declared({}).find((i: any) => i.key === 'lmstudioBaseUrl');
    ...
    test('settings.get fallback chain: DB → env → declared default', async () => {
        ...
        ctx.fns.settings.set({ module: 'llm', scopeType: 'global', key: 'defaultModel', value: 'db-model' });
        expect(ctx.fns.settings.get({ module: 'llm', scopeType: 'global', key: 'defaultModel' }))
            .toBe('db-model');

        // 4. remove DB row → falls back to env again
        ctx.fns.settings.remove({ module: 'llm', scopeType: 'global', key: 'defaultModel' });
        expect(ctx.fns.settings.get({ module: 'llm', scopeType: 'global', key: 'defaultModel' }))
            .toBe('env-model');
    });
    ...
    test('env parsing respects descriptor.type for number/boolean', async () => {
        ...
        const v = ctx.fns.settings.get({ module: 'agent', scopeType: 'global', key: 'debounceMs' });
        expect(v).toBe(750);
    ...
    test('getNumber uses declared default when present, no fallback needed', async () => {
        const ctx = await mkTestCtx();
        const ms = ctx.fns.settings.getNumber({ module: 'agent', scopeType: 'global', key: 'debounceMs' });
        expect(ms).toBe(1000);
    });

    test('getString fallback param still works for un-declared keys', async () => {
        const ctx = await mkTestCtx();
        const v = ctx.fns.settings.getString({
