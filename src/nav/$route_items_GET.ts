...
export default async function (ctx: Context, _session: Session | null, opts: { req: Request; params: Record<string, string> }) {
    ...
    const row = (item: any) => {
        ...
        if (agent) {
            ...
            const badge = Number(agent.unread ?? 0) > 0
                ...
                : "";
            return `<a href="${esc(item.href)}" class="nav-row flex min-h-7 items-center gap-1.5 rounded px-1.5 py-0.5 text-left outline-none hover:bg-gray-100">
  ${ctx.fns.ui.modelLogo({ model: agent.model, active, bare: true, compact: true })}
  <span class="min-w-0 flex-1 truncate text-xs text-gray-700">${esc(agent.title || agent.id)} <span class="font-mono text-[10px] font-normal text-gray-400">(${esc(agent.id)})</span></span>
  ${badge}
</a>`;
        }
        return `<a href="${esc(item.href)}" class="nav-row flex min-h-8 items-center gap-2 rounded px-2 py-1 text-sm outline-none hover:bg-gray-50">
  <i class="ph ${group(item) === "Projects & files" ? "ph-folder" : group(item) === "Plugins" ? "ph-plugs" : "ph-gear"} shrink-0 text-gray-400"></i>
  <span class="min-w-0 flex-1 truncate">${esc(item.label)}</span>
  ${item.hint ? `<span class="max-w-32 shrink-0 truncate text-[10px] text-gray-400">${esc(item.hint)}</span>` : ""}
</a>`;
    };
    let html: string;
  ...
  ${list.map(agentRow).join("")}
    ...
    const projects = () => {
        ...
        return folders.map(([dir, list]) => `<a href="/files?path=${encodeURIComponent(dir)}" class="nav-row flex min-h-8 items-center gap-2 rounded px-2 py-1 text-sm outline-none hover:bg-gray-50">
  <i class="ph ph-folder-open shrink-0 text-gray-400"></i>
  <span class="min-w-0 flex-1 truncate">${esc(workspaceLabel(dir))}</span>
  <span class="shrink-0 text-[10px] text-gray-400">${list.length} ${list.length === 1 ? "agent" : "agents"}</span>
</a>`).join("");
    };
        ...
        html = `<div class="grid grid-cols-1 divide-y divide-gray-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">${groups.map(name => {
            ...
            const content = name === "Chats"
                ...
                : name === "Projects & files"
                    ...
                    : own.map(row).join("");
            return `<section class="min-w-0 p-2.5"><h3 class="mb-1 px-2 text-[10px] font-semibold uppercase tracking-wider text-gray-400">${name}</h3>${content || `<div class="px-2 py-2 text-xs text-gray-300">empty</div>`}</section>`;
        }).join("")}</div>`;
    }
    return new Response(html || `<div class="px-4 py-5 text-sm text-gray-400">nothing</div>`, {
