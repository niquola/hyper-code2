...
export default function (ctx: Context, _session: Session | null, opts: { prompt: any; error?: string }): string {
    ...
    const form = `<form hx-popup="secureInput.submit" title="${esc(p.title)}" class="mt-4 space-y-4">
      ...
      <div class="flex justify-end gap-2">
        ${ctx.fns.procs.ui.button({ action: 'cancel-secure-input', label: 'Cancel', tone: 'ghost', title: `Cancel ${p.title}`, attrs: { 'data-secure-cancel': true, onclick: `window.hyperPopup?.invoke('secureInput.submit', ${JSON.stringify({ id: String(p.id), cancel: '1' })})` } })}
