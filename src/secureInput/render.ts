...
export default function (ctx: Context, _session: Session | null, opts: { prompt: any; error?: string }): string {
    ...
    const form = `<form hx-popup="secureInput.submit" title="${esc(p.title)}" class="mt-4 space-y-4">
      ...
      <div class="flex justify-end gap-2">
        <button type="button" hx-popup="secureInput.submit" hx-popup-params='{"id":"${esc(p.id)}","cancel":"1"}' title="Cancel ${esc(p.title)}" class="inline-flex items-center justify-center rounded-xl border border-gray-300 bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-200 focus:outline-none focus:ring-4 focus:ring-gray-200">Cancel</button>
