...
export default async function (
  ...
  ${module.skill ? `<section class="mt-7 rounded-xl border border-base-300 bg-base-100 p-4"><div class="flex items-center justify-between gap-3"><div><h2 class="font-medium">Documentation</h2><p class="mt-0.5 text-xs text-base-content/50">Instructions available to the agent</p></div><a href="/procs/modules/skill?name=${encodeURIComponent(module.name)}" class="rounded-lg border border-base-300 px-3 py-1.5 text-xs hover:border-primary hover:text-primary">Open SKILL.md</a></div></section>` : ""}
</section></div>` };
}
