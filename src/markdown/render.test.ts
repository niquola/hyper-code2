...
describe("markdown.render", () => {
    ...
    test("mermaid blocks are pre-rendered before markdown html", async () => {
        const mermaid = mock(async (_ctx: Context, code: string) => {
            return "<div class=\"mermaid-diagram\" data-code=\"" + code.replace(/"/g, "&quot;") + "\"><svg></svg></div>";
        });
        const ctx = { fns: { markdown: { highlight, mermaid } } } as unknown as Context;
        const html = await render(ctx, ["# Diagram", "", "```mermaid", "flowchart LR", "A --> B", "```"].join("\n"));
    ...
    test("mermaid render failure falls back to plain code block", async () => {
        ...
        const ctx = { fns: { markdown: { highlight, mermaid } } } as unknown as Context;
        const html = await render(ctx, ["```mermaid", "flowchart LR", "A --> B", "```"].join("\n"));
        expect(html).toContain("<pre><code>");
