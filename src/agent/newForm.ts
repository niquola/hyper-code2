...
export default async function (ctx: Context, _session: Session | null, _opts: {}): Promise<string> {
    ...
    return `<input name="title" maxlength="120" placeholder="title (optional)" class="w-full rounded border px-3 py-2"><select name="model" class="w-full rounded border bg-white px-3 py-2 font-mono">${options}</select><div><input name="workspaceDir" list="workspace-dirs" value="${esc(process.cwd())}" required hx-get="/agent/dirs/status" hx-trigger="load, keyup changed delay:250ms" hx-target="#workspace-dir-status" class="w-full rounded border px-3 py-2 font-mono"><datalist id="workspace-dirs"></datalist><div id="workspace-dir-status" class="mt-1 min-h-5 text-[11px]"></div></div><details class="rounded border"><summary class="p-2">Base system prompt · ${Math.ceil((base.core?.length || 0) / 4)}t</summary><pre class="border-t bg-gray-50 p-3 text-[11px] whitespace-pre-wrap">${esc(base.core)}</pre></details>${presetHtml}<textarea name="systemPrompt" rows="4" placeholder="custom instructions (optional)" class="w-full rounded border px-3 py-2 font-mono"></textarea>`;
}
