...
describe("agent.finishTask", () => {
    test("child with task metadata can finish successfully", async () => {
        ...
        const res = finishTask(ctx, null, { agent, summary: "done", result: { value: 1 } });
    ...
    test("result saved into scratchpad metadata", async () => {
        ...
        agent.scratchpad.delegateTask = { parentId: "parent1", mode: "await", status: "running" };
        ctx.fns.session.save({ agent });
        finishTask(ctx, null, { agent, summary: "done", result: { value: 2 } });
    ...
    test("waiter resolves in await mode", async () => {
        ...
        const res = finishTask(ctx, null, { agent, summary: "done", result: { value: 3 } });
    ...
    test("missing delegateTask metadata throws", async () => {
        ...
        const agent = ctx.fns.agent.start({ model: "m", systemPrompt: "" });
        ctx.fns.session.save({ agent });
        expect(() => finishTask(ctx, null, { agent, summary: "done" })).toThrow("finishTask: missing delegateTask metadata");
    });
    ...
    test("async mode does not resolve waiter", async () => {
        ...
        let resolved: any = null;
        ctx.state.delegateTaskWaiters = new Map([[agent.id, { resolve: (v: any) => { resolved = v; } }]]);
        const res = finishTask(ctx, null, { agent, summary: "done", result: { value: 4 } });
    ...
    test("summary required", async () => {
        ...
        agent.scratchpad.delegateTask = { parentId: "parent1", mode: "await", status: "running" };
        ctx.fns.session.save({ agent });
        expect(() => finishTask(ctx, null, { agent, summary: "   " })).toThrow("finishTask: summary is required");
    });
});
