...
describe("agent.compact", () => {
    test("replaces last §result:* user message with summary", async () => {
        ...
        const agent = ctx.fns.agent.start({ model: "x" });
        agent.messages.push(
        ...
        const res = compact(ctx, null, { agent, summary: "listed 42 files" });
    ...
    test("returns replaced:false when no tool-result exists", async () => {
        ...
        expect(compact(ctx, null, { agent, summary: "s" }).replaced).toBe(false);
    });
    ...
    test("targets the MOST RECENT result when several exist", async () => {
        ...
        ctx.fns.session = { replaceMessages: (_c: any, _s: any, opts: { id: string; messages: any[] }) => { agent.messages = opts.messages; }, syncAgentState: () => agent };
        compact(ctx, null, { agent, summary: "summary" });
    ...
    describe("with {message, summary} — compact from index onward", () => {
        test("drops messages from index onward and inserts a synthetic user note", async () => {
            ...
            ctx.fns.session = { replaceMessages: (_c: any, _s: any, opts: { id: string; messages: any[] }) => { agent.messages = opts.messages; }, syncAgentState: () => agent };
            const res = compact(ctx, null, { agent, message: 2, summary: "explored A/B/C dead-ends" });
            expect(res.replaced).toBe(true);
        ...
        test("walks back when from-1 is an assistant marker", async () => {
            ...
            const res = compact(ctx, null, { agent, message: 4, summary: "tool B too long" });
        ...
        test("invalid index → replaced:false", async () => {
            ...
            const agent = ctx.fns.agent.start({ model: "x" });
            agent.messages.push({ role: "user", content: "hi" });
            expect(compact(ctx, null, { agent, message: 99, summary: "x" }).replaced).toBe(false);
            expect(compact(ctx, null, { agent, message: -1, summary: "x" }).replaced).toBe(false);
        });
    });
});
