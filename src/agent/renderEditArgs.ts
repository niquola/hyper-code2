...
export default async function (
    ...
    return `<div class="edit-preview"><div class="mb-3 flex items-center gap-2"><i class="ph ph-file-code text-base-content/40"></i><span class="min-w-0 flex-1 truncate font-mono text-xs font-medium text-base-content/70">${esc(path)}</span><span class="text-[10px] text-base-content/40">${edits.length} change${edits.length === 1 ? "" : "s"}</span></div><div class="space-y-3">${cards.join("") || `<div class="rounded-lg border border-ui-border bg-base-100 p-3 text-xs text-base-content/40">No edits</div>`}</div></div>`;
}
