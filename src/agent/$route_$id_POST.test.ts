...
function userMessages(ctx: any, id: string): any[] {
  return ctx.fns.session.getMessages({ id }).filter((m: any) => m.role === 'user');
}

function nextRunAt(ctx: any, id: string): any {
  const row = (ctx.fns.procs.db.select({ sql: 'SELECT next_run_at FROM agents WHERE id = ?', params: [id] }) as any[])[0];
...
describe('POST /agent/:id', () => {
  test('plain JSON client gets simplified ack', async () => {
    ...
    expect(userMessages(ctx, agent.id).map((m: any) => m.content)).toEqual(['hi']);
    expect(calls.find(c => c[0] === 'wakeWorker')).toBeTruthy();
    expect(nextRunAt(ctx, agent.id)).toBeTruthy();
  });
  ...
  test('htmx submit gets 204 No Content', async () => {
    ...
    expect(userMessages(ctx, agent.id).map((m: any) => m.content)).toEqual(['hello via htmx']);
  });
  ...
  test('plain browser HTML form submit redirects 303 back to /agent/:id', async () => {
    ...
    expect(userMessages(ctx, agent.id).map((m: any) => m.content)).toEqual(['from a form']);
  });
  ...
  test('multi-field form (no `text`) is serialized to "name: value" lines', async () => {
    ...
    expect(res.status).toBe(303);
    expect(userMessages(ctx, agent.id).map((m: any) => m.content)).toEqual(['name: Иван\nage: 30\nnote: hello']);
  });
  ...
  test('text field wins over other fields when both present', async () => {
    ...
    expect(userMessages(ctx, agent.id).map((m: any) => m.content)).toEqual(['explicit']);
  });
});
