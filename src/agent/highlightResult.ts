...
export default async function (ctx: Context, _session: Session | null, opts: { output: string }): Promise<string> {
    const { output } = opts;
    ...
    return await ctx.fns.markdown.highlight({ code: output, lang: 'javascript' });
}
