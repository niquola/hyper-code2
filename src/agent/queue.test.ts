...
async function drainUntilIdle(ctx: any, deadlineMs = 5000) {
    ...
    while (Date.now() - t0 < deadlineMs) {
        const busy = ctx.fns.procs.db.select({
...
describe('agent queue (state on agents row)', () => {
    test('POST sets next_run_at, worker drains, advances cursor', async () => {
        ...
        const a = ctx.fns.agent.start({ model: 'm' });
        ...
        ctx.fns.procs.db.run({ sql: 'UPDATE agents SET next_run_at = ? WHERE id = ?', params: [Date.now(), a.id] });

        await drainUntilIdle(ctx);

        expect(seen).toEqual([a.id]);
        const row = ctx.fns.procs.db.select({ sql: 'SELECT run_state, next_run_at, last_processed_msg_idx FROM agents WHERE id = ?', params: [a.id] })[0];
    ...
    test('debounced run is held until next_run_at <= now', async () => {
        ...
        ctx.fns.procs.db.run({ sql: 'UPDATE agents SET next_run_at = ? WHERE id = ?', params: [t0 + 150, a.id] });
    ...
    test('multiple agents drained serially', async () => {
        ...
        const a1 = ctx.fns.agent.start({ model: 'm' });
        const a2 = ctx.fns.agent.start({ model: 'm' });
        ctx.fns.session.save({ agent: a1 });
        ctx.fns.session.save({ agent: a2 });

        await ctx.fns.session.appendUserMessage({ id: a1.id, text: 'a1' });
        await ctx.fns.session.appendUserMessage({ id: a2.id, text: 'a2' });
        const now = Date.now();
        ctx.fns.procs.db.run({ sql: 'UPDATE agents SET next_run_at = ? WHERE id IN (?, ?)', params: [now, a1.id, a2.id] });
    ...
    test('successful run does NOT reschedule itself just because run() appended assistant/tool messages', async () => {
        ...
        ctx.fns.agent.run = async (c: any, _s: any, opts: any) => {
            ...
            const msgs = c.fns.session.getMessages({ id: agent.id });
            for (const m of msgs) if (m.role === 'user') seenUserTexts.push(m.content);
            c.fns.session.appendMessage({ id: agent.id, message: { role: 'assistant', content: 'reply' } });
        };
        ...
        const row = ctx.fns.procs.db.select({ sql: 'SELECT run_state, next_run_at FROM agents WHERE id = ?', params: [a.id] })[0];
        expect(row.run_state).toBe('idle');
        expect(row.next_run_at).toBeNull();
    });
    ...
    test('cursor advances past pre-existing messages on success (regression: backfill seeded cursor)', async () => {
        ...
        ctx.fns.procs.db.run({ sql: 'UPDATE agents SET last_processed_msg_idx = ? WHERE id = ?', params: [1, a.id] });
        ...
        const row = ctx.fns.procs.db.select({ sql: 'SELECT last_processed_msg_idx FROM agents WHERE id = ?', params: [a.id] })[0];
