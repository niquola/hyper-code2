...
async function setup() {
    ...
    ctx.fns.files.write = async (c: any, _s: any, opts: { path: string; content: string }) => {
        ((c.state as any).__written ??= {})[opts.path] = opts.content;
        return { ok: true };
    };
    ctx.fns.files.read = async (_c: any, _s: any, opts: { path: string }) =>
        opts.path === 'src/x.ts' ? ['a', 'b', 'c', 'd', 'e'].join('\n') : `READ:${opts.path}`;
    ctx.fns.files.readHashline = async (_c: any, _s: any, opts: any) => ({ text: `READHASH:${opts.path}:${opts.startLine ?? ''}:${opts.endLine ?? ''}:${opts.maxLines ?? ''}` });
    ...
    ctx.fns.files.editHashline = async (_c: any, _s: any, opts: { input: string }) => {
