...
Как сделать так, чтобы перезапуск сервера перестал быть решением с последствиями.
Ничего из описанного ещё не реализовано — это план, записанный, чтобы с ним можно было
спорить до того, как появится код.
...
5. **Атомарный захват.** `UPDATE agents SET run_state='running', run_started_at=…
   WHERE id IN (SELECT id … WHERE run_state='idle' AND next_run_at <= now LIMIT 1)
   RETURNING id`. Одну строку может забрать только один запрос — это и есть гарантия
   «один исполнитель на агента».
...
**Что уже правильно.** Намерение пишется до действия (шаг 7). Захват атомарен (шаг 5).
Выделение `idx` устойчиво к гонкам (шаг 2). Есть даже аренда: `recoverStaleRuns`
освобождает прогоны, чей `run_started_at` старше 10 минут.

**Чего не хватает — ровно три вещи.**

- Аренда считает **возраст захвата, а не признаки жизни**: честный долгий ход старше
  десяти минут неотличим от мёртвого, а упавший короткий ждёт освобождения десять минут.
  Лечится heartbeat'ом.
- Не записано, **кто** держит прогон. Поэтому нельзя отличить «его ведёт другой процесс»
  от «его не ведёт никто» — и после убийства сервера `oc` и `ehk` остались `running`.
- Заглушка шага 7 при падении **остаётся заглушкой навсегда**: в переписке висит «is
  running», хотя никто уже не бежит, и агент не знает, случилось действие или нет.

Всё остальное в этом документе — достройка этих трёх дыр.
