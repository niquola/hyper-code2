...
export default async function (ctx: Context, _session: Session | null, opts: {
    ...
    const topicArray=opts.topics?.length?`{${opts.topics.map(x=>`"${String(x).replace(/\\/g,"\\\\").replace(/"/g,'\\"')}"`).join(",")}}`:null;
    await ctx.fns.procs.db.run({sql:`INSERT INTO news.items(id,title,url,image_url,author,points,comments,topics,article_md,summary,summary_long,query,source,source_label,fetched_at,shown_at)
      VALUES(?,?,?,?,?,?,?,?::text[],?,?,?,?,?,?,coalesce(?::timestamptz,now()),?::timestamptz)
      ON CONFLICT(id) DO UPDATE SET title=excluded.title,url=coalesce(excluded.url,news.items.url),image_url=coalesce(excluded.image_url,news.items.image_url),author=coalesce(excluded.author,news.items.author),points=coalesce(excluded.points,news.items.points),comments=coalesce(excluded.comments,news.items.comments),topics=coalesce(excluded.topics,news.items.topics),article_md=coalesce(excluded.article_md,news.items.article_md),summary=coalesce(excluded.summary,news.items.summary),summary_long=coalesce(excluded.summary_long,news.items.summary_long),query=coalesce(excluded.query,news.items.query),source=excluded.source,source_label=coalesce(excluded.source_label,news.items.source_label),shown_at=coalesce(excluded.shown_at,news.items.shown_at)`,params:[opts.id,opts.title,opts.url??null,opts.imageUrl??null,opts.author??null,opts.points??null,opts.comments??null,topicArray,opts.articleMarkdown??null,opts.summary??null,opts.summaryLong??null,opts.query??null,opts.source,opts.sourceLabel??null,opts.fetchedAt??null,opts.shownAt??null]});
    return {id:opts.id};
}
