...
**Extractable types are data, not code.** `knowledge.extractionSchema` derives them from the graph: an `Entity/<Type>` record with `extract: true` (mixins excluded) is extractable; its `required` list, `extract_hint` and `anonymous` flag are rendered into the prompt and enforced by the writer. Anonymous types (`Event`, `EventParticipation`) carry a synthesized `name` that need not appear in the evidence, are never matched by label, and are skipped (cascading to dependants) unless every required attribute/relation is evidenced. `Attribute.vocabulary` (root Concept) restricts a reference attribute to the root's `isA`/`subClassOf` descendants — the writer rejects other targets and the quote need not contain the concept title. `date` values are `YYYY-MM-DD`, `datetime` values are `YYYY-MM-DD` or ISO date-time; both are evidenced by the quote containing the ISO literal, the year or the day number. Adding a new type or attribute in Knowledge is enough to extract it. Anonymous records are deduplicated deterministically by `knowledge.matchAnonymous` before creation: identity = the type's required fields (dates compare by day); an all-reference identity (EventParticipation = event × participant) matches outright, a scalar identity (Event = kind × start) additionally needs one shared non-Concept link (organizer/about or a participant reported in the same batch) — one corroborated candidate is reused, several are `ambiguous` (skipped), disjoint links mean a distinct record, unknown links skip conservatively.
